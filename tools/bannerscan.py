...
class bannerscan(threading.Thread):
    ...
    def run(self):
        ...
        for port in PORTS:
            ...
            try:
                print(("[*] %s\r" % (site[0:60].ljust(60, " "))), end=' ')
                ...
                for path in PATHS:
                    try:
                        url = site + path
                        print(("[*] %s\r" % (url[0:60].ljust(60, " "))), end=' ')
    ...
    for ip in ips:
        ...
        if type(out[ip]) == type(dict()):
            for port in list(out[ip].keys()):
                ...
                for key in list(out[ip][port]["headers"].keys()):
